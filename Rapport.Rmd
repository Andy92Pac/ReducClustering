...
#161 153 234 145 111  72  77 186  72 229
```

20 classes :

```{r,eval=FALSE, message=FALSE, warning=FALSE, include=FALSE,results='hide'}
kmeans.coil20 = NbClust(data = coil.data, method = "kmeans",min.nc = 15,max.nc = 20,index = c("ccc","db","silhouette"))
table(effectif20 = kmeans.coil20$Best.partition)
#47  18 127  96  74 136  48 103  72  26  64  34  35  58  59  72 144 72  83  72
```
...
#36 277  24   5 308  23   6   7 425  26  11   4 216  72
```
...
table(effectif = average.coil20$Best.partition)
```
...
#371 159 109 225 216  72  72  72  72  72
```
...
table(effectif = ward.coil20$Best.partition)
#43  56  60 140  80  79 109  76  77  52  36  56  72  72  72  72  72 72  72  72
```
...
#39 186 212  48  67   5 162   4 365  36  24   4 288
```
...
table(effectif = complete.coil20$Best.partition)
#30 100  60  41 152  39   5 158   4   7   4  86   9 365  28  36  24 4 102 186
```
...
```{r}
specc.coil@size
#68  72 105  72  76  72  41  18   6 162  32  72 103  90  62  27  27 214
#75  46
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=FALSE,results='hide'}
EM.Coil <- mixmodCluster(coil.data, dataType = "quantitative", models = mixmodMultinomialModel("Binary_pk_Ekj"),criterion = c("NEC","BIC","ICL"), strategy = new ("Strategy",algo = "EM"), nbCluster = 10)
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=FALSE,results='hide'}
EM.Jaffe <- mixmodCluster(jaffe.data, dataType = "quantitative", models = mixmodMultinomialModel("Binary_pk_Ekj"),criterion = c("NEC","BIC","ICL"), strategy = new ("Strategy",algo = "EM"), nbCluster = 10)
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=FALSE,results='hide'}
cluspca.coil <- cluspca(coil.data,method = c("RKM","FKM"),nclus = 1,)
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=FALSE,results='hide'}
cluspca.jaffe <- cluspca(jaffe.data,method = c("RKM","FKM"),nclus = 1,)
```
