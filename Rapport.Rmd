...
```{r, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE, results='hide',echo='FALSE'}
hcpc.coil <- HCPC(pca.coil,nb.clust = 20,graph = TRUE)
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
hcpc.Jaffe <- HCPC(pca.jaffe,nb.clust = 10,graph = TRUE)
```
...
EM.coil <- mixmodCluster(as.data.frame(coil.data), models = all, strategy = new("Strategy",algo = "EM"), nbCluster = 20)
mixmodCluster(data.frame(pca.coil$ind), 2:20,dataType = "quantitative",criterion = "BIC")
View(EM.coil@bestResult@partition)
```
...
```{r}
hcpcPartition.coil <- hcpc.coil20
...
```{r}
MclustPartition.coil <- Mclust.jaffe
...
```{r}
MclustDRpartition.coil <- MclustDR.jaffe
...
```{r}
hcpcPartition.jaffe <- hcpc.jaffe
