...
**Obs** : Ci-dessous on retrouve une visualisation du plan d'individus factorielle issu de l'ACP. On peut vaguement distingué 4 grands foyers.
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
EM.mfeat <- mixmodCluster(data.frame(pca.mfeat$ind), 1:10,dataType = "quantitative",criterion = "BIC",strategy = new("Strategy",algo = "EM"))
```


```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
mclust.mfeat = Mclust(mfeat.data, G = 10)
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
EM.mnist <- mixmodCluster(data.frame(pca.mnist$ind), 1:10,dataType = "quantitative",criterion = "BIC",strategy = new("Strategy",algo = "EM"))
```
