---
title: "Rapport"
author: "Farid Ahamada & Andy Mpondo Black"
output:
...
Nous avons pour but dans ce projet de mettre en corrélation nos compétences acquises dans le cours de réduction de dimensionS ainsi qu'en modèle de mélange. En appliquant des algotithmes sur différents jeu de données qui ont pour point commun être des bases de données d'images. Une fois ces algorithmes lancé nous devions faire un travail d'analyse, d'interprêtations et de comparaison pour trouver selon nous ce qui nous semble être les solutions les plus cohérentes et meilleures en termes de clustering. 
...
**Multi-Feature digit (MFEAT1)** : Cet ensemble de données est constitué de caractéristiques de chiffres manuscrits (0 à 9) extrait d'une collection de cartes d'utilité néerlandaises. 200 motifs par classe (pour un total de 2 000 modèles) ont été numérisés en binaire. Ce jeu données est composé de 2000 observations et 240 variables.
...
Chaque datasets est au format .mat ils contiennent tous une colonne "Y" correspondants à la classe ainsi que des features "X".

Nous avons ces variables X et Y assignées dans différentes variables pour plus de facilité et de clarté dans notre code.
...
Contrairement à l'ACP du jeu de données Coil on remarque une répartition des individus très distincte ce qui dans un premier temps peut nous renseigner sur le fait que les individus ne sont pas identiques ou colporte la même informations. Car si c'était le cas nous aurions pu voir que tous les individus sont correlé à un axes et seraient confondu en une unique masse.
...
**Obs** : Visuellement, on peut vaguement distinguer 5 grands foyers.
...
Il s'agit d'une représentation des individus catégorisé de par leurs valeurs du cos2. On remarque les points présentant des similarités sont regroupés. On remarque 6 foyers. (Nous précisons les foyers ne font en  aucun cas allusion à des classes)
...
Dim 1 = 6,19% variance expliquée (valeurs propres)
Dim 2 = 4,48% variance expliquée (valeurs propres)
...
Graphe illisible du fait du nombre d'individus et des relations qu'il y a entre chaque.
...
Dim 1 = 16,13% variance expliquée (valeurs propres)
Dim 2 = 9,76% variance expliquée (valeurs propres)
...
Comme pour le jeu de données Mnist, la quantité élevée d'individu ainsi que de relations entre ceux ci rend compliquée la visualisation.
...
**Rem** : Nous avons détaillé notre méthodologie de travail pour Coil data --> Kmeans afin de montrer comment nous sommes parvenus aux résultats obtenu ainsi que les deux différentes structure trouvées (cluster auto et cluster initialisé à un certain nombre de classes). Pour le reste nous avons juste affiché les résultats car la méthode est la même.
...
**Obs** : Les algorithmes sont lancés uniquement qu'en se basant sur l'index silhouette pour cause d'erreur avec d'autres index qui font planté la bonne éxecutions de ces algorithmes. 
...
**Rem** : Dans cette partie nous avons paramétré la fonction HCPC à -1 afin qu'elle nous propose pour elle le meilleure découpage du dendogramme. Mais nous avons aussi initialisé au bon nombre de classes afin de voir pouvoir comparer par la suite les effectifs.
...
Nous devons initialisé notre algorithme par un nombre de clusters souhaité. Nous allons directement l'initialisé aux nombres de classes adéquats c'est à dire 20 pour le jeu de données Coil. Nous allons voir en terme d'effectifs s'il arrive à classer les individus au mieux donc plus proche de la réalité.
...
Pour choisir le nombre de classes nous allons nous baser sur ce que nous avons réalisé au préalable.

Nous avons réalisé des méthodes en mode qu'on qualifier "d'automatique"" c'est à dire qu'on a laissé les algorithmes nous proposer selon leurs calculs un nombre de clusters ainsi qu'une répartition de la population. (Nbclust et HCPC). Puis nous avons initialisé par nous même le nombre de clusters avec le spectrale.
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
EM.mfeat <- mixmodCluster(data.frame(pca.mfeat$ind), 8:10,dataType = "quantitative",criterion = "BIC",strategy = new("Strategy",algo = "EM"))
```
...
```{r}
em.mfeatEff <- matrix(c(150, 116, 80, 495, 169, 238, 146, 245, 132, 229),nrow = 1)
em.mfeatEff
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
mclust.mfeat = Mclust(mfeat.data, G = 8:10)
```

**Effectif** : 

```{r}
mclust.mfeatEff <- matrix(c(1986,    2,    2,    2,    2,    2 , 2,  2 ),nrow = 1)
mclust.mfeatEff
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
EM.mnist <- mixmodCluster(data.frame(pca.mnist$ind), 8:10,dataType = "quantitative",criterion = "BIC",strategy = new("Strategy",algo = "EM"))
```

**Effectif** :

```{r}
em.mnistEff <- matrix(c(854, 310, 128, 102, 911, 215, 229, 236, 151, 359),nrow = 1)
em.mnistEff
```


```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
mclust.mnist = Mclust(mnist.data, G = 8:10)
```

**Effectif** : 

```{r}
mclust.mnistEff <- matrix(c(486, 584, 209, 424, 372, 133, 215, 561, 285, 226),nrow = 1)
mclust.mnistEff
```
...
```{r}
mclustdr.mfeatEff <- matrix(c(1986,  2, 2,  2, 2, 2, 2, 2 ),nrow = 1)
mclustdr.mfeatEff
```
...
```{r}
mclustdr.mnistEff <- matrix(c(486, 584, 209, 424, 372, 133, 215, 561, 285, 226),nrow = 1)
mclustdr.mnistEff
```
...
```{r}
complete.mfeatScore <- data.frame(specificity=0.8726,sensitivity=0.4937,precision=0.3,recall=0.4937,NMI=0.4994,entropy=0.4114,purity=0.4675,row.names = "score")
complete.mfeatScore
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
hcpcPartition.mfeat <- hcpc.mfeat$data.clust$clust
...
```{r}
hcpc.mfeatScore <- data.frame(specificity=0.9273,sensitivity=0.5551,precision=0.4576,recall=0.5551,NMI=0.6019,entropy=0.3711,purity=0.6105,row.names = "score")
hcpc.mfeatScore
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
speccPartition.mfeat <- spect.mfeat@.Data
...
```{r}
specc.mfeatScore <- data.frame(specificity=0.9705,sensitivity=0.8939,precision=0.7701,recall=0.8939,NMI=0.8743,entropy=0.1053,purity=0.865,row.names = "score")
specc.mfeatScore
```
...
```{r}
em.mfeatScore <- data.frame(specificity=0.9059,sensitivity=0.46,precision=0.3509,recall=0.46,NMI=0.5162,entropy=0.4416,purity=0.556,row.names = "score")
em.mfeatScore
```
...
res = external_validation(as.vector(mfeat.label),as.numeric(MclustPartition.mfeat), method = "nmi", summary_stats = T)
```

**Mesure de performances**

```{r}
mclust.mfeatScore <- data.frame(specificity=0.9861,sensitivity=0.014,precision=0.0996,recall=0.9861,NMI=0.0137,entropy=0.017,purity=0.107,row.names = "score")
mclust.mfeatScore
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
MclustDRpartition.mfeat <- mclustdr.mfeat$class
table(MclustDR = MclustDRpartition.mfeat,label = mfeat.label)
res = external_validation(as.vector(mfeat.label),as.numeric(MclustDRpartition.mfeat), method = "nmi", summary_stats = T)
```

**Mesure de performances**

```{r}
mclustdr.mfeatScore <- data.frame(specificity=0.014,sensitivity=0.9861,precision=0.0996,recall=0.9861,NMI=0.0137,entropy=0.017,purity=0.107,row.names = "score")
mclustdr.mfeatScore
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
kmeansPartition.mnist <- kmeans.mnist$Best.partition
...
```{r}
kmeans.mnistScore <- data.frame(specificity=0.9099,sensitivity=0.5282,precision=0.3946,recall=0.5282,NMI=0.4999,entropy=0.4154,purity=0.5648,row.names = "score")
kmeans.mnistScore
```
...
```{r}
average.mnistScore <- data.frame(specificity=0.2314,sensitivity=0.915,precision=0.1169,recall=0.915,NMI=0.1973,entropy=0.0865,purity=0.21,row.names = "score")
average.mnistScore
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
wardPartition.mnist <- ward.mnist$Best.partition
...
```{r}
ward.mnistScore <- data.frame(specificity=0.9357,sensitivity=0.5433,precision=0.4844,recall=0.5433,NMI=0.6023,entropy=0.3797,purity=0.6209,row.names = "score")
ward.mnistScore
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
completePartition.mnist <- complete.mnist$Best.partition
...
```{r}
complete.mnistScore <- data.frame(specificity=0.6781,sensitivity=0.5929,precision=0.1699,recall=0.5929,NMI=0.3259,entropy=0.3604,purity=0.3634,row.names = "score")
complete.mnistScore
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
hcpcPartition.mnist <- hcpc.mnist$data.clust$clust
...
```{r}
hcpc.mnistScore <- data.frame(specificity=0.9042,sensitivity=0.4233,precision=0.3293,recall=0.4233,NMI=0.4173,entropy=0.5318,purity=0.4916,row.names = "score")
hcpc.mnistScore
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
speccPartition.mnist <- spect.mnist@.Data
...
```{r}
specc.mnistScore <- data.frame(specificity=0.8996,sensitivity=0.7001,precision=0.4367,recall=0.7001,NMI=0.6389,entropy=0.2833,purity=0.6286,row.names = "score")
specc.mnistScore
```
...
```{r}
em.mnistScore <- data.frame(specificity=0.8495,sensitivity=0.2703,precision=0.1664,recall=0.2703,NMI=0.2387,entropy=0.6593,purity=0.3139,row.names = "score")
em.mnistScore
```
...
res = external_validation(as.vector(mnist.label),as.numeric(MclustPartition.mnist), method = "nmi", summary_stats = T)
```

**Mesure de performances**

```{r}
mclust.mnistScore <- data.frame(specificity=0.92,sensitivity=0.4641,precision=0.3919,recall=0.4641,NMI=0.4848,entropy=0.484,purity=0.5725,row.names = "score")
mclust.mnistScore
```
...
```{r,eval=FALSE, message=FALSE, warning=FALSE, include=TRUE,results='hide'}
MclustDRpartition.mnist <- mclustdr.mnist$class
table(MclustDR = MclustDRpartition.mnist,label = mnist.label)
res = external_validation(as.vector(mnist.label),as.numeric(MclustDRpartition.mnist), method = "nmi", summary_stats = T)
```

**Mesure de performances**

```{r}
mclustdr.mnistScore <- data.frame(specificity=0.92,sensitivity=0.4641,precision=0.3919,recall=0.4641,NMI=0.4848,entropy=0.484,purity=0.5725,row.names = "score")
mclustdr.mnistScore
```
...
Pour le jeu de données Coil l'algorithme de Ward est celui qui fit au mieux les data tandis que pour le jeu de données Jaffe il s'agit de l'agorithme Mclust qui fit le mieux les data et arrive à perçevoir la meilleure structure des classes. En ce qui concerne le jeu de données, l'algorithme de Ward est également le plus approprié. Enfin, pour le jeu de données Mnist, notre étude nous laisse penser que Ward est une nouvelle fois la méthode la plus efficace.
